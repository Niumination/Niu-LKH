...
const actionColors = {
  cyan: 'from-cyan-600/20 to-cyan-600/5 border-cyan-600/20 hover:border-cyan-500/30',
...
export default function Dashboard() {
  ...
  const totalLocations = useMemo(() => new Set(entries.map((e) => e.tempat)).size, [entries])

  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
      <Card as="section" className="relative overflow-hidden bg-gradient-to-br from-cyber-800 via-cyber-900 to-cyber-950">
        <div className="absolute -top-20 -right-20 w-80 h-80 bg-cyan-500/5 rounded-full blur-3xl" aria-hidden="true" />
        <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-purple-500/5 rounded-full blur-3xl" aria-hidden="true" />
        <div className="relative z-10 p-6 lg:p-8">
          <div className="flex items-center gap-2 mb-2">
            <LayoutDashboard className="w-5 h-5 text-cyan-400" aria-hidden="true" />
            <span className="text-xs text-cyan-400 font-medium uppercase tracking-wider">Niu-LKH v3.2</span>
          </div>
      ...
      {entries.length > 0 ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={BarChart3} value={totalWorkdays} label="Hari Kerja" color="from-cyan-500 to-blue-500" />
          <StatCard icon={CalendarDays} value={totalHolidays} label="Libur/Cuti" color="from-amber-500 to-orange-500" />
          <StatCard icon={MapPin} value={totalLocations} label="Lokasi" color="from-green-500 to-emerald-500" />
          <StatCard icon={TrendingUp} value={monthlyProgress.length} suffix="bulan" label="Periode Aktif" color="from-purple-500 to-pink-500" />
        </div>
      ) : (
        <Card as="section" className="p-8 text-center">
          <FileSpreadsheet className="w-12 h-12 text-slate-600 mx-auto mb-3" aria-hidden="true" />
          <h3 className="text-white font-semibold mb-2">Belum Ada Data LKH</h3>
          <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
            Kamu bisa langsung isi manual lewat form, upload file Excel untuk preview, atau muat data LKH yang sudah disiapkan (Januari — Juni 2026).
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            ...
            >
              <ClipboardList className="w-4 h-4" aria-hidden="true" /> Isi LKH Baru
            </Link>
            <button
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" /> : <Download className="w-4 h-4" aria-hidden="true" />}
              Muat Data LKH
            </button>
            <Link
              to="/excel-preview"
            ...
            >
              <FileSpreadsheet className="w-4 h-4" aria-hidden="true" /> Preview Excel
            </Link>
          </div>
        </Card>
      )}
      ...
      >
        <span className="flex items-center gap-1 text-slate-300 text-sm">
          ...
          Data sample Januari — Juni 2026.
        </span>
        <p className="text-slate-300 text-sm mt-2">
          Akan memuat <strong className="text-cyan-400">181 entri</strong> LKH untuk Afrizal Munthe. Entri yang sudah ada tidak akan ditimpa.
        </p>
      </Modal>
        ...
        <>
          <div className="flex items-center justify-between">
            <h2 className="text-white font-semibold text-lg flex items-center gap-2">
              <CalendarDays className="w-4 h-4 text-cyan-400" aria-hidden="true" />
              Progress Bulanan
            </h2>
            <Link to="/stats" className="text-xs text-cyan-400 hover:text-cyan-300 transition-colors flex items-center gap-1">
              Detail Statistik <TrendingUp className="w-3 h-3" aria-hidden="true" />
            </Link>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {monthlyProgress.map((m) => (
              <Card key={m.month} className="p-5 hover:border-cyan-500/20 transition-all group">
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-cyan-600 to-cyan-400 rounded-full transition-all duration-700" style={{ width: `${Math.max((m.workdays / m.total) * 100, 5)}%` }} />
                  </div>
                  <div className="flex justify-between text-xs">
                  ...
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-amber-600 to-amber-400 rounded-full transition-all duration-700" style={{ width: `${Math.max((m.holidays / m.total) * 100, 5)}%` }} />
                  </div>
                </div>

                <div className="flex items-center gap-3 text-[10px] text-slate-500 pt-2 border-t border-slate-800">
                  <span className="flex items-center gap-1">
                    <MapPin className="w-3 h-3" aria-hidden="true" /> {m.locations} lokasi
                  </span>
                  <span className="flex items-center gap-1">
                    <CalendarDays className="w-3 h-3" aria-hidden="true" /> {m.total} hari
                  </span>
                </div>
              </Card>
            ))}
...
function ActionCard({ to, icon: Icon, label, sub, color }) {
  const c = actionColors[color] || actionColors.cyan
  return (
    <Link to={to} className={`bg-gradient-to-br ${c} rounded-xl p-4 transition-all group`}>
      <Icon className="w-5 h-5 text-cyan-400 mb-2 group-hover:scale-110 transition-transform" aria-hidden="true" />
